...
TEST_F(FsZenFSTest, SimpleAuxDirRename) {
  ...
  ASSERT_TRUE(status.ok()) << "Directory '" << initial_dir_name << "' could not be renamed to '" << renamed_dir_name << "'";

  status = fs_->FileExists(initial_dir_name, options, &dbg);
  ASSERT_TRUE(!status.ok()) << "Directory '" << initial_dir_name << "' still exists after renaming it";

  status = fs_->FileExists(renamed_dir_name, options, &dbg);
  ASSERT_TRUE(status.ok()) << "Directory '" << renamed_dir_name << "' does not exists";
};
...
TEST_F(FsZenFSTest, SimpleZenfsFileRename) {
  ...
  ASSERT_TRUE(status.ok()) << "Failed to create NewWritableFile '" << initial_file_name << "'";

  status = fs_->RenameFile(initial_file_name, renamed_file_name, options, &dbg);
  ASSERT_TRUE(status.ok()) << "File '" << initial_file_name << "' could not be renamed to '" << renamed_file_name << "'";

  status = fs_->FileExists(initial_file_name, options, &dbg);
  ASSERT_TRUE(!status.ok()) << "File '" << initial_file_name << "' still exists after renaming it";

  status = fs_->FileExists(renamed_file_name, options, &dbg);
  ASSERT_TRUE(status.ok()) << "File '" << renamed_file_name << "' does not exists";
};
...
TEST_F(FsZenFSTest, DirectoriesWithGrandchildrenRename) {
  ...
  std::string child_file_path = std::filesystem::path(initial_dir_name) / child_file_name;
  status = fs_->NewWritableFile(child_file_path, foptions, &child_file, &dbg);
  ASSERT_TRUE(status.ok()) << "Failed to create NewWritableFile '" << child_file_path << "'";

  std::string child_dir_path = std::filesystem::path(initial_dir_name) / child_dir_name;
  status = fs_->CreateDir(child_dir_path, options, nullptr);
  ASSERT_TRUE(status.ok()) << "Failed to create directory '" << child_dir_path << "'";

  std::string grandchild_file_path = std::filesystem::path(child_dir_path) / grandchild_file_name;
  status = fs_->NewWritableFile(grandchild_file_path, foptions, &grandchild_file, &dbg);
  ASSERT_TRUE(status.ok()) << "Failed to create NewWritableFile '" << grandchild_file_path << "'";

  status = fs_->RenameFile(initial_dir_name, renamed_dir_name, options, &dbg);
  ASSERT_TRUE(status.ok()) << "Directory '" << initial_dir_name << "' could not be renamed to '" << renamed_dir_name << "'";

  std::string renamed_child_file_path = std::filesystem::path(renamed_dir_name) / child_file_name;
  status = fs_->FileExists(renamed_child_file_path, options, &dbg);
  ASSERT_TRUE(status.ok()) << "File '" << renamed_child_file_path << "' does not exists";

  status = fs_->FileExists(child_file_path, options, &dbg);
  ASSERT_TRUE(!status.ok()) << "File '" << child_file_path << "' still exists after renaming it";
  ...
  std::string renamed_child_dir_path = std::filesystem::path(renamed_dir_name) / child_dir_name;
  status = fs_->FileExists(renamed_child_dir_path, options, &dbg);
  ASSERT_TRUE(status.ok()) << "Directory '" << renamed_child_dir_path << "' does not exists";

  status = fs_->FileExists(child_dir_path, options, &dbg);
  ASSERT_TRUE(!status.ok()) << "Directory '" << child_dir_path << "' still exists after renaming it";

  std::string renamed_grandchild_file_path = std::filesystem::path(renamed_child_dir_path) / grandchild_file_name;
  status = fs_->FileExists(renamed_grandchild_file_path, options, &dbg);
  ASSERT_TRUE(status.ok()) << "File '" << renamed_grandchild_file_path << "' does not exists";

  status = fs_->FileExists(grandchild_file_path, options, &dbg);
  ASSERT_TRUE(!status.ok()) << "File '" << grandchild_file_path << "' still exists after renaming it";
};

TEST_F(FsZenFSTest, SimilarDirectoriesRename) {
  // The directory initial_dir_name is renamed.
  // initial_dir_name is a substring of unrelated_dir_name, which should not get renamed including its childen.
  // Directory structure:
  // aux_dir/
  // ├───- test/ (->renamed)
  // |     └── testfile
  ...
  ASSERT_TRUE(status.ok()) << "Failed to create directory '" << unrelated_dir_name << "'";

  std::string nested_file_path = std::filesystem::path(initial_dir_name) / nested_file_name;
  status = fs_->NewWritableFile(nested_file_path, foptions, &nested_file, &dbg);
  ASSERT_TRUE(status.ok()) << "Failed to create NewWritableFile '" << nested_file_path << "'";

  std::string unrelated_file_path = std::filesystem::path(unrelated_dir_name) / unrelated_file_name;
  status = fs_->NewWritableFile(unrelated_file_path, foptions, &unrelated_file, &dbg);
  ASSERT_TRUE(status.ok()) << "Failed to create NewWritableFile '" << unrelated_file_path << "'";

  status = fs_->RenameFile(initial_dir_name, renamed_dir_name, options, &dbg);
  ASSERT_TRUE(status.ok()) << "Directory '" << initial_dir_name << "' could not be renamed to '" << renamed_dir_name << "'";

  std::string renamed_nested_file_path = std::filesystem::path(renamed_dir_name) / nested_file_name;
  status = fs_->FileExists(renamed_nested_file_path, options, &dbg);
  ASSERT_TRUE(status.ok()) << "File '" << renamed_nested_file_path << "' does not exists";

  status = fs_->FileExists(nested_file_path, options, &dbg);
  ASSERT_TRUE(!status.ok()) << "File '" << nested_file_path << "' still exists after renaming it";
  ...
  ASSERT_TRUE(status.ok()) << "Directory '" << unrelated_dir_name << "' does not exists";

  status = fs_->FileExists(unrelated_file_path, options, &dbg);
  ASSERT_TRUE(status.ok()) << "Directory '" << unrelated_file_path << "' does not exists";
};
